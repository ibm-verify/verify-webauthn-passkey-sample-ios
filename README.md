# IBM Verify Sample App for Passkey on iOS

An implementation of Apple Passkeys with IBM Verify as the relying party FIDO service.
...
1. Open Terminal and clone the repository and open the project file in Xcode.
   ```
   git clone https://github.com/ibm-verify/webauthn-passkey-sample-ios.git
   xed .
   ```
...
2. In the project **Signing & Capabilities**, update the following settings to suit your development environment:
   ...
   The value of the associated domain will contain the replying party identifier generated by your IBM Verify (ISV) tenant or IBM Verify Access (ISVA) on-premise configuration.  For example: `webcredentials:example.com`
